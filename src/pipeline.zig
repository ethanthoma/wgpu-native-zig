...
pub const CreatComputePipelineAsyncCallback = *const fn (status: CreatePipelineAsyncStatus, pipeline: ?*ComputePipeline, message: ?[*:0]const u8, userdata: ?*anyopaque) callconv(.C) void;
...
test "chain method compiles" {
    _ = &(PrimitiveState{ .topology = PrimitiveTopology.triangle_list, .strip_index_format = IndexFormat.uint16, .front_face = FrontFace.ccw, .cull_mode = CullMode.back }).withDepthClipControl(false);
}
